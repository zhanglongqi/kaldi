...
DecodeUtteranceLatticeFasterClass::DecodeUtteranceLatticeFasterClass(
		...
		LatticeWriter *lattice_writer, double *like_sum, // on success, adds likelihood to this.
		...
		decoder_(decoder), decodable_(decodable), trans_model_(&trans_model), word_syms_(
				word_syms), utt_(utt), acoustic_scale_(acoustic_scale), determinize_(
				determinize), allow_partial_(allow_partial), alignments_writer_(
				alignments_writer), words_writer_(words_writer), compact_lattice_writer_(
				compact_lattice_writer), lattice_writer_(lattice_writer), like_sum_(
				like_sum), frame_sum_(frame_sum), num_done_(num_done), num_err_(
				num_err), num_partial_(num_partial), computed_(false), success_(
				false), partial_(false), clat_(NULL), lat_(NULL)
{
}
...
void DecodeUtteranceLatticeFasterClass::operator ()()
{
	...
	if (determinize_)
	{
		...
		if (!DeterminizeLatticePhonePrunedWrapper(*trans_model_, lat_,
				decoder_->GetOptions().lattice_beam, clat_,
...
bool DecodeUtteranceLatticeFaster(
		...
		std::string utt,
		double acoustic_scale, //Charles: not necessary
		bool determinize, //Charles: not necessary
		bool allow_partial, //Charles: not necessary
		Int32VectorWriter *alignment_writer,
		Int32VectorWriter *words_writer,
		CompactLatticeWriter *compact_lattice_writer, //Charles: not necessary
		LatticeWriter *lattice_writer, //Charles: not necessary
		double *like_ptr)// puts utterance's like in like_ptr on success. //Charles: not necessary
{
	using fst::VectorFst;

	//Charles: decoder.Decode(&decodable) is the main function!!!
	if (!decoder.Decode(&decodable))
	{
		;
	}

	/*   //Charles: original code below
	 ...
	 //Charles: original code above */

	//TODO: Charles: does it do something? decoder.ReachedFinal()
	//Charles: just to check if decode is finished
	if (!decoder.ReachedFinal())
	{
		;
		;
...
//  double likelihood;//Charles: not necessary

	LatticeWeight weight;

	//Charles: num_freams is used for determinization. not necessary.
//	int32 num_frames;

	{ // First do some stuff with word-level traceback...
		VectorFst<LatticeArc> decoded;

		//Charles: no use, as explained below
...
//		num_frames = alignment.size();
		...
		{
			std::cerr << utt << ' ';
			//Charles: decode result is a text array, which are stored in words
			for (size_t i = 0; i < words.size(); i++)
			{
...
//    likelihood = -(weight.Value1() + weight.Value2());//Charles: not necessary
	}

//Charles comment start:
	/*
	 // Get lattice, and do determinization if requested.
	 Lattice lat;
	 ...
	 */
	//charles end;
	//Charles: just some outputs
	/*
	 ;
	 ;
	 */
	//Charles end;
//  *like_ptr = likelihood; //Charles: not necessary
	return true;
}
...
bool DecodeUtteranceLatticeSimple(
		...
		Int32VectorWriter *words_writer,
		CompactLatticeWriter *compact_lattice_writer,
		LatticeWriter *lattice_writer, double *like_ptr)
{ // puts utterance's like in like_ptr on success.
	...
	{ // First do some stuff with word-level traceback...
		...
		VectorFst<LatticeArc> decoded;
		//Charles: get best path and store it to decoded;
		if (!decoder.GetBestPath(&decoded))
			// Shouldn't really reach this point as already checked success.
			;
	...
	{
		CompactLattice clat;
		if (!DeterminizeLatticePhonePrunedWrapper(trans_model, &lat,
				decoder.GetOptions().lattice_beam, &clat,
				decoder.GetOptions().det_opts))
			;
			// We'll write the lattice without acoustic scaling.
		if (acoustic_scale != 0.0)
...
void AlignUtteranceWrapper(const AlignConfig &config, const std::string &utt,
		...
		int32 *num_done, int32 *num_error, int32 *num_retried, double *tot_like,
		int64 *frame_count)
{
